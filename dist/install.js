...
import { intro, outro, select, multiselect, spinner, isCancel, cancel } from "@clack/prompts";
...
function addMcpToConfig(ide, mcpPath) {
...
function removeMcpFromConfig(ide) {
    const configPath = getConfigPath(ide);
    ...
    if (ide === "opencode")
        key = "mcp";
    if (config[key] && config[key].unifymemfile) {
        delete config[key].unifymemfile;
        fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
    }
}
async function main() {
